...
import com.live.xue.living.PublishActivity;
import com.live.xue.own.OwnFragment;
...
//        }
       ...
       String name = (String) SharePrefsUtils.get(this, "user", "user_nicename","user");
        ...
        ll_recycle.post(new Runnable() {
            @Override
            public void run() {
                int width = DensityUtils.screenHeight(GameActivity.this);
                int view_height =( width-DensityUtils.getStatusBarHeight(GameActivity.this) - DensityUtils.dip2px(GameActivity.this,65))*2/5;
...
//                ll_recycle.setLayoutParams(params);
                item_height = view_height/6;
                //params.width -1 params.height 584 item_height 97
                GameLeftAdapter adapter = new GameLeftAdapter(new ArrayList<List<String>>());
                if (Logs.isDebug) {
                    rv_game_left.setLayoutManager(new GridLayoutManager(GameActivity.this,11));
                    rv_game_left.setAdapter(adapter);
                }
...
//        List<List<String>> chart = GameResult.mokeData().getData().getChart();
                GameRightAdapter Rightadapter = new GameRightAdapter(chart);
                 ...
                 width = DensityUtils.screenWidth(GameActivity.this);
                int rowCount = ((width-item_height*12) / item_height)+1;
                if (Logs.isDebug) {
                    rv_game_right.setLayoutManager(new GridLayoutManager(GameActivity.this,rowCount));
                    rv_game_right.setAdapter(Rightadapter);
                }
//                Logs.e("params.width "+ll_recycle.getWidth() +" params.height "+ll_recycle.getHeight()+" item_height "+item_height+"  "+width);
                Logs.e("rowCount "+rowCount+  "  width " +width+" item_height"+item_height);
            }
        });
    }
